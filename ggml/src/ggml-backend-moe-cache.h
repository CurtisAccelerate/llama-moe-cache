...
struct ggml_moe_cache_api {
    ...
    // Thread-0 CPU miss-loop wall time after GPU dispatch.
    void (*miss_time)(int dev, int64_t wall_us);

    // Bounds-safe GLU skip mask for more than 64 routed rows.
    int (*glu_hits_w)(const void * src0_data, const void * src1_data,
                      void * dst_data, size_t dst_nb1, int ith,
